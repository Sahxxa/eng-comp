...
volatile int sistema_travado = 0; // 0 = Livre, 1 = Alguém já apertou
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
...
int main(void)
{
  ...
  Utility_Init();
	...
	GPIO_Pin_Mode(GPIOA, PIN_0, INPUT); // Botão K_UP
...
void EXTI4_IRQHandler(void) {
    // Verifica se a interrupção foi na linha 4 e se o sistema está livre
    if (!sistema_travado) {
        GPIO_Write_Pin(GPIOA, PIN_6, LOW);
        sistema_travado = 1;              // Bloqueia o sistema
    }
    HAL_GPIO_EXTI_IRQHandler(GPIO_PIN_4); // Limpa a flag de interrupção
}
/* USER CODE END 4 */
